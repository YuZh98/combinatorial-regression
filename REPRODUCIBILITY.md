...
Table 2 (RMSE for estimating $\beta$ suing MH-Within-Gibbs): all quantities can be generated by running ```make full``` with custom settings (see README.md for details)
...
Only run_pmmh.py is the interactive preface. You can change variable values in step1 and step2 in run_pmmh.py and run the following in the terminal:
```bash
